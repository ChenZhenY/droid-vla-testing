...
# def maybe_update_instruction(current_prompt: str) -> str:
#     """
#     If the user presses 'i' or 'I' (without hitting Enter), pause the loop,
#     ask for a new prompt (with input()), and return it. Otherwise return the
#     original prompt unchanged.
#     """
#     fd = sys.stdin.fileno()
#     old_attrs = termios.tcgetattr(fd)
#     try:
#         # Non-blocking single-key polling
#         tty.setcbreak(fd)
#         ready, _, _ = select.select([sys.stdin], [], [], 0)
#         if ready:
#             ch = sys.stdin.read(1)
#             if ch and ch.lower() == 'i':
#                 # Restore cooked mode before using input()
#                 termios.tcsetattr(fd, termios.TCSADRAIN, old_attrs)
#                 try:
#                     new_prompt = input("\nNew instruction/prompt: ").strip()
#                     if new_prompt:
#                         print(f"[prompt] Updated.")
#                         return new_prompt
#                 finally:
#                     # Re-enter cbreak so subsequent polls keep working
#                     tty.setcbreak(fd)
#     finally:
#         # Always restore original terminal settings at the end of this call
#         termios.tcsetattr(fd, termios.TCSADRAIN, old_attrs)
